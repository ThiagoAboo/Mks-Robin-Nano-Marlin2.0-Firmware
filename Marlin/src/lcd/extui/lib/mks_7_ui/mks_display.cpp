...
void mks_lv_register() {
  ...
  disp_drv.flush_cb = mks_disp_flush; /*Set your driver function*/
...
void mks_draw_error_message(PGM_P const msg) {
  ...
  if (msg) mks_disp_string((TFT_WIDTH - strlen(msg) * 16) / 2, 100, msg, 0xFFFF, 0x0000);
  mks_disp_string((TFT_WIDTH - strlen("PRINTER HALTED") * 16) / 2, 140, "PRINTER HALTED", 0xFFFF, 0x0000);
  mks_disp_string((TFT_WIDTH - strlen("Please Reset") * 16) / 2, 180, "Please Reset", 0xFFFF, 0x0000);
}

void mks_disp_flush(lv_disp_drv_t * disp, const lv_area_t * area, lv_color_t * color_p) {
...
void mks_disp_string(uint16_t x, uint16_t y, const char * string, uint16_t charColor, uint16_t bkColor) {
  while (*string != '\0') {
    mks_disp_char_1624(x, y, *string, charColor, bkColor);
    string++;
    x += 16;
  }
}

static const uint16_t MKS_ASCII_Table_16x24[] PROGMEM = {
  // Space ' '
...
void mks_disp_char_1624(uint16_t x, uint16_t y, uint8_t c, uint16_t charColor, uint16_t bkColor) {
  for (uint16_t i = 0; i < 24; i++) {
    const uint16_t tmp_char = pgm_read_word(&MKS_ASCII_Table_16x24[((c - 0x20) * 24) + i]);
